...
class ADXRegimeEngine:
    ...
    def _classify_direction(
        self, plus_di: float, minus_di: float
    ) -> RegimeDirection:
    ...
    def _composite_label(
        strength: RegimeStrength, direction: RegimeDirection
    ) -> str:
    ...
    def _build_results(
        ...
        for i in range(len(adx)):
            ...
            di_spread = (pdi_v - mdi_v) if (not np.isnan(pdi_v) and not np.isnan(mdi_v)) else float("nan")
            regime    = self._composite_label(strength, direction)

            results.append(ADXResult(
                adx       = adx_v,
                plus_di   = pdi_v,
                minus_di  = mdi_v,
                dx        = dx_v,
...
def adx_result_to_dict(result: ADXResult) -> dict:
    ...
    return {
        ...
        "minus_di":  round(result.minus_di, 4)  if not np.isnan(result.minus_di)  else None,
        "dx":        round(result.dx, 4)         if not np.isnan(result.dx)         else None,
        "tr":        round(result.tr, 6),
        "di_spread": round(result.di_spread, 4) if not np.isnan(result.di_spread) else None,
